...
class node:
	...
	def __init__(self):
		self.param = [[random.uniform(0, 1) for i in range(node.dim)], node.sigma]
	def dist(self, x):
...
class network:
	...
	def leastSquares(self, trainX, trainY):
		...
		self.w = np.dot(pinv(phi), npTrainY)
	...
	def CL(self, trainX, trainY, lr = 0.2, maxIter = 10000, deadNode = True):
		if deadNode == False:
			for k in range(maxIter):
			...
			coWinners = 4
			samples = random.sample(range(len(trainX)), len(trainX))
			for k in range(maxIter):
				index = samples[k % len(trainX)]
				dists = np.array([self.nodes[i].dist(trainX[index]) for i in range(self.n)])
